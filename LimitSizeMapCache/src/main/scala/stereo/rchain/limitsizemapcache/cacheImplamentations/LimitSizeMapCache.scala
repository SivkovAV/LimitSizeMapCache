...
/**
  ...
  */
case class LimitSizeMapCacheState[K, V](
  ...
  private def moveItemOnTop(key: K): LimitSizeMapCacheState[K, V] = {
    if (mayBeTopKey.isDefined && mayBeTopKey.get == key)
      ...
      val mayBeNextItem = for {
        nk <- mapValue.mayBeNextKey; pk = mapValue.mayBePrevKey; r = nk -> newItems(nk).setPrevKey(pk)
      } yield r
      val mayBePrevItem = for {
        pk <- mapValue.mayBePrevKey; nk = mapValue.mayBeNextKey; r = pk -> newItems(pk).setNextKey(nk)
      } yield r
  ...
  private def removeOldItem(): LimitSizeMapCacheState[K, V] = {
    ...
    LimitSizeMapCacheState(
      maxItemCount,
      itemCountAfterSizeCorrection,
      items - key,
      mayBeTopKey,
      items(key).mayBeNextKey
    )
  }
  ...
  private def removeOldItems(itemCountForRemoving: Int): LimitSizeMapCacheState[K, V] = {
    if (0 < itemCountForRemoving) {
      ...
      LimitSizeMapCacheState[K, V](
        state.maxItemCount,
        state.itemCountAfterSizeCorrection,
        newItems,
        state.mayBeTopKey,
        state.mayBeBottomKey
      )
    } else this.copy()
  }
